...
enum SolverType { FiniteElement, DirectFGT, DirectTreeFGT, ImprovedFGT };
...
RuntimeParameters::RuntimeParameters()
  ...
  prm->declare_entry("dim", "2", Patterns::Integer(1, 3),
  ...
  prm->declare_entry("conservative", "true", Patterns::Bool(),
                     "Whether threshold preserves volume of each phase "
                     "(Allen-Cahn vs Cahn-Hilliard).");
  prm->declare_entry("initial_time", "0.", Patterns::Double(0.),
                     "The initial time of the simulation. ");
  prm->declare_entry("final_time", "1.", Patterns::Double(0.),
                     "The final time of the simulation. ");
  prm->declare_entry("dt", "0.001", Patterns::Double(0.),
                     "The time step size for MBO.");
  prm->declare_entry("n_sub_steps", "1", Patterns::Integer(1, 100),
                     "The number of time steps to take to solve heat equation "
                     "in each MBO steps.");
  prm->declare_entry(
      "solver type", "FiniteElement",
      Patterns::Selection("FiniteElement|DirectFGT|DirectTreeFGT|ImprovedFGT"),
      "The heat equation solver, choices are "
      "<FiniteElement|DirectFGT|DirectTreeFGT|ImprovedFGT>.");
  prm->declare_entry("droptol", "1e-6", Patterns::Double(0.),
                     "The cut off tolerance in multipole expansion.");
  prm->declare_entry("theta", "1.0", Patterns::Double(0.),
                     "The parameter for theta-scheme, can be explicit Euler "
                     "(0), implicit Euler (1), Crank-Nicolson (0.5), etc.");
  prm->declare_entry("theta_s", "60", Patterns::Double(0.),
                     "The static contact angle, in degree.");

  prm->declare_entry("adaptation_interval", "5", Patterns::Integer(1, 100),
                     "The number of MBO steps between two AMRs.");
  prm->declare_entry("n_adaptation_sweeps", "5", Patterns::Integer(1, 100),
                     "The number of sweeps for each adaptation.");
  prm->declare_entry("max_adaptation_level", "8", Patterns::Integer(0, 20),
                     "Maximal difference in level between the most coarse and "
                     "the most fine cells.");
  prm->declare_entry("n_initial_global_refines", "2", Patterns::Integer(0, 15),
                     "The number of global refines we do on the mesh before "
                     "start adaptive refinement. ");
  prm->declare_entry("n_initial_adaptive_refines", "8",
                     Patterns::Integer(0, 15),
                     "The number of adaptive refines we do on the globally "
                     "refined mesh before start of time integration.");
  prm->declare_entry("degree", "1", Patterns::Integer(1, 5),
                     "The polynomial degree for the finite element space.");
  prm->declare_entry(
      "smoother_sweeps", "2", Patterns::Integer(1, 100),
      "The number of smoother sweeps used in AMG preconditioner.");
  prm->declare_entry("aggregation_threshold", "0.02", Patterns::Double(0.),
                     "(ad hoc) Larger aggregation_threshold will decrease the "
                     "number of iterations, but increase the costs per "
                     "iteration");
  prm->declare_entry("eps", "1e-12", Patterns::Double(0.),
...
void RuntimeParameters::parse_parameters() {
  ...
  prm->enter_subsection("Algorithm");
  {
    ...
    const std::string solver_type = prm->get("solver type");
    if (solver_type.compare("FiniteElement") == 0) {
      solver = FiniteElement;
    } else if (solver_type.compare("DirectFGT") == 0) {
      solver = DirectFGT;
    } else if (solver_type.compare("DirectTreeFGT") == 0) {
      solver = DirectTreeFGT;
    } else if (solver_type.compare("ImprovedFGT") == 0) {
      solver = ImprovedFGT;
    } else {
      AssertThrow(false, dealii::ExcNotImplemented());
    }
    droptol = prm->get_double("droptol");
