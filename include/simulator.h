...
class Simulator {
 ...
 private:
  ...
  void run_fe();
  void run_direct_fgt();
  void run_direct_tree_fgt();
  void run_improved_fgt();

  void update_fgt_data();
  ...
  fgt::Matrix source_points_;
  fgt::Matrix target_points_;

  // Eigen::VectorXd
  fgt::Vector areas_;
  fgt::Vector weights0_;
  fgt::Vector weights1_;
  fgt::Vector weights2_;
...
void          Simulator<dim>::initialize_simulator() {
  ...
  const unsigned int max_grid_level =
      prm_->n_initial_global_refines + prm_->max_adaptation_level;
  ...
  for (unsigned int l = 0; l < prm_->n_initial_adaptive_refines; l++) {
    ...
    // Calculate criteria
    {
      ...
      for (unsigned int cell_no = 0; cell != endc; cell++, cell_no++) {
        ...
        criteria(cell_no) = std::log(
            1 + std::sqrt(du0[0] * du0[0] + du1[0] * du1[0] + du2[0] * du2[0]));
      }
    }
...
void Simulator<dim>::compute_volumes(std::vector<double> &volumes) {
  ...
  std::vector<double> u0_values(n_q_points), u1_values(n_q_points),
      u2_values(n_q_points);
...
void          Simulator<dim>::refine_mesh() {
  ...
  parallel::distributed::SolutionTransfer<dim, TrilinosWrappers::MPI::Vector>
      solution_trans0(dof_handler_);
  parallel::distributed::SolutionTransfer<dim, TrilinosWrappers::MPI::Vector>
      solution_trans1(dof_handler_);
  parallel::distributed::SolutionTransfer<dim, TrilinosWrappers::MPI::Vector>
      solution_trans2(dof_handler_);
  triangulation_.prepare_coarsening_and_refinement();
...
void          Simulator<dim>::print_system_info() {
  ...
  deallog << "----------------------------------------" << std::endl
          << "Number of active cells: " << triangulation_.n_active_cells()
          << " (on " << triangulation_.n_levels() << " levels)" << std::endl
...
/**
 * Simple threshold. Set the phase to be the one with larger value, and solid
 * (0) is unchanged.
 */
template <int dim>
...
/**
 * Conservative threshold. Find a level set to preserve volumes of non-solid
 * phases.
 */
template <int dim>
...
void Simulator<dim>::assemble_system_matrix(const double ddt) {
  ...
  for (; cell != endc; cell++) {
    ...
    constraints_.distribute_local_to_global(local_matrix, local_dof_indices,
                                            system_matrix_);
  }
...
void Simulator<dim>::assemble_system_rhs(const double ddt) {
  ...
  std::vector<double> u0_values(n_q_points), u1_values(n_q_points),
      u2_values(n_q_points);
  std::vector<Tensor<1, dim>> grad_u0_values(n_q_points),
      grad_u1_values(n_q_points), grad_u2_values(n_q_points);
  ...
  for (; cell != endc; cell++) {
    ...
    for (unsigned int q = 0; q < n_q_points; q++) {
      ...
      for (unsigned int i = 0; i < dofs_per_cell; i++) {
        local_rhs0(i) += (fe_values.shape_value(i, q) * u0_values[q] -
                          ddt * (1.0 - theta) * (fe_values.shape_grad(i, q) *
                                                 grad_u0_values[q])) *
                         fe_values.JxW(q);
    ...
    cell->get_dof_indices(local_dof_indices);
    constraints_.distribute_local_to_global(local_rhs0, local_dof_indices,
                                            system_rhs0_);
    constraints_.distribute_local_to_global(local_rhs1, local_dof_indices,
                                            system_rhs1_);
    constraints_.distribute_local_to_global(local_rhs2, local_dof_indices,
                                            system_rhs2_);
  }
}
...
void          Simulator<dim>::solve_system() {
  ...
  SolverCG<TrilinosWrappers::MPI::Vector> cg0(
      solver_control0,
      SolverCG<TrilinosWrappers::MPI::Vector>::AdditionalData());
  SolverCG<TrilinosWrappers::MPI::Vector> cg1(
      solver_control1,
      SolverCG<TrilinosWrappers::MPI::Vector>::AdditionalData());
  SolverCG<TrilinosWrappers::MPI::Vector> cg2(
      solver_control2,
      SolverCG<TrilinosWrappers::MPI::Vector>::AdditionalData());
...
void          Simulator<dim>::output_solution() {
  ...
  const std::string filename =
      ("output/solution-" + Utilities::int_to_string(timestep_number_, 4));
  std::ofstream output((filename + ".vtu").c_str());
  data_out.write_vtu(output);
}

template <int dim>
void          Simulator<dim>::update_fgt_data() {
  // Update points, areas, weights and bandwidth, assuming
  // valid data in u0, u1, u2
  //
  // source_points are active cell centers
  // target_points are active vertices
  source_points_.resize(triangulation_.n_active_cells(), dim);
  target_points_.resize(dof_handler_.n_dofs(), dim);
  areas_.resize(triangulation_.n_active_cells());
  weights0_.resize(triangulation_.n_active_cells());
  weights1_.resize(triangulation_.n_active_cells());
  weights2_.resize(triangulation_.n_active_cells());

  bandwidth_ = 2. * std::sqrt(prm_->dt);

  const dealii::QMidpoint<dim> quadrature_formula;
  const dealii::UpdateFlags    update_flags = dealii::update_values;
  dealii::FEValues<dim>        fe_v(mapping_, dof_handler_.get_fe(),
                             quadrature_formula, update_flags);
  std::vector<double> u0_value(1), u1_value(1), u2_value(1);

  for (auto cell = dof_handler_.begin_active(); cell != dof_handler_.end();
       cell++) {
    fe_v.reinit(cell);
    fe_v.get_function_values(u0_, u0_value);
    fe_v.get_function_values(u1_, u1_value);
    fe_v.get_function_values(u2_, u2_value);

    const auto cell_no = cell->active_cell_index();
    for (unsigned int d = 0; d < dim; d++) {
      source_points_(cell_no, d) = cell->center()(d);
    }

    for (unsigned int v = 0; v < dealii::GeometryInfo<dim>::vertices_per_cell;
         v++) {
      const auto vertex_no = cell->vertex_dof_index(v, 0);
      for (unsigned int d = 0; d < dim; d++) {
        target_points_(vertex_no, d) = cell->vertex(v)(d);
      }
    }

    areas_(cell_no) = cell->measure();

    weights0_(cell_no) = areas_(cell_no) * u0_value[0] *
                         std::pow(4. * M_PI * prm_->dt, -dim * 0.5);
    weights1_(cell_no) = areas_(cell_no) * u1_value[0] *
                         std::pow(4. * M_PI * prm_->dt, -dim * 0.5);
    weights2_(cell_no) = areas_(cell_no) * u2_value[0] *
                         std::pow(4. * M_PI * prm_->dt, -dim * 0.5);
  }
}
...
void          Simulator<dim>::run() {
  const SolverType solver = prm_->solver;

  if (solver == FiniteElement) {
    run_fe();
  } else if (solver == DirectFGT) {
    run_direct_fgt();
  } else if (solver == DirectTreeFGT) {
    run_direct_tree_fgt();
  } else if (solver == ImprovedFGT) {
    run_improved_fgt();
  } else {
...
template <int dim>
void          Simulator<dim>::run_direct_fgt() {

  deallog << "Running the MBOX simulator with direct FGT solver." << std::endl;
  AssertThrow(timestep_number_ == 0, ExcNotInitialized());

  AssertThrow(prm_->degree == 1,
              dealii::ExcMessage("Degree must be 1 for FGT solvers."));

  {
    ...
    while (time < prm_->final_time) {
      time += dt;
      timestep_number_++;
      deallog << "Time step " << timestep_number_ << " on t = " << time
              << std::endl;

      timer.enter_subsection("Heat solver");
      update_fgt_data();
      fgt::Direct fast_gauss_transform(source_points_, bandwidth_);
      deallog << "  Performing Gauss transform .." << std::endl;
      tmp0 = fast_gauss_transform.compute(target_points_, weights0_);
      tmp1 = fast_gauss_transform.compute(target_points_, weights1_);
      tmp2 = fast_gauss_transform.compute(target_points_, weights2_);
      deallog << "  Done." << std::endl;
      for (unsigned int i = 0; i < dof_handler_.n_dofs(); i++) {
        u0_(i) = tmp0(i);
        u1_(i) = tmp1(i);
        u2_(i) = tmp2(i);
      }
...
template <int dim>
void          Simulator<dim>::run_direct_tree_fgt() {

  deallog << "Running the MBOX simulator with direct tree FGT solver."
          << std::endl;
  AssertThrow(timestep_number_ == 0, ExcNotInitialized());
...

  {
    ...
    while (time < prm_->final_time) {
      ...
      timer.enter_subsection("Heat solver");
      update_fgt_data();
      fgt::DirectTree fast_gauss_transform(source_points_, bandwidth_,
                                           prm_->droptol);
      deallog << "  Performing Gauss transform .." << std::endl;
      tmp0 = fast_gauss_transform.compute(target_points_, weights0_);
      tmp1 = fast_gauss_transform.compute(target_points_, weights1_);
...
template <int dim>
void          Simulator<dim>::run_improved_fgt() {

  deallog << "Running the MBOX simulator with improved FGT solver."
          << std::endl;
  AssertThrow(timestep_number_ == 0, ExcNotInitialized());
...

  {
    ...
    while (time < prm_->final_time) {
      ...
      timer.enter_subsection("Heat solver");
      update_fgt_data();
      fgt::Ifgt fast_gauss_transform(source_points_, bandwidth_, prm_->droptol);
      deallog << "  Performing Gauss transform .." << std::endl;
      tmp0 = fast_gauss_transform.compute(target_points_, weights0_);
      tmp1 = fast_gauss_transform.compute(target_points_, weights1_);
      tmp2 = fast_gauss_transform.compute(target_points_, weights2_);
...
template <int dim>
void          Simulator<dim>::run_fe() {

  deallog << "Running the MBOX simulator with FE solver." << std::endl;
  AssertThrow(timestep_number_ == 0, ExcNotInitialized());

  {
    ...
    while (time < prm_->final_time) {
      ...
      timer.enter_subsection("Heat solver");
      for (unsigned substep_number = 0; substep_number < prm_->n_sub_steps;
           substep_number++) {
